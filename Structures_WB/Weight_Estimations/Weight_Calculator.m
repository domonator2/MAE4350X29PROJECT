function [W_wing, W_emp, W_fus, W_n, W_g, W_str, W_cp] = Weight_Calculator(C)

% Wing Weight Calculation
W_wing = 0.80*(3.08 *( ((C.Kw*C.n_ult*C.W_GW)/C.tc_max) * ( (tan(C.sweep_LE) - 2*(1-C.lambda_wing)/(C.AR_wing*(1+C.lambda_wing)) )^2 +1)*10E-6 )^0.593 * (C.AR_wing*(1+C.lambda_wing))^0.89 *(C.Sw)^0.741);
